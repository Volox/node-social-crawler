...
function Twitter( config ) {
  ...
  var api = new Twit( {
    ...
    access_token: this.keys.token, // eslint-disable-line camelcase
    access_token_secret: this.keys.tokenSecret, // eslint-disable-line camelcase
  } );
...
Twitter.prototype.get = function*( data, options ) {
  ...
  if( options.since ) {
    data.q += ' since: '+moment( options.since ).format( 'YYYY-MM-DD' );
  }
  if( options.until ) {
    data.q += ' until: '+moment( options.until ).format( 'YYYY-MM-DD' );
  }

  while( numPages<MAX_PAGES ) {
    var result = null;
    try {
      // this.trace( 'Query with data: ', data );
      ...
      this.error( 'Request error', requestError );
      break;
    }
    var tweets = result[ 0 ].statuses;

    if( tweets && tweets.length===0 ) {
      this.debug( 'No more medias' );
  ...
  this.trace( 'Retrieved %d pages for the query', numPages );
...
Twitter.prototype.getByGeoGrid = function*( gridPoints, options, status ) {
  ...
  for( var i=start; i<gridPoints.length; i+=bulk ) {
    var array = _.times( bulk, _.partial( map, i, bulk ), this );
    yield array;
    this.status( 'done', 'grid', i, i+bulk );
  }
};
