...
Crawler.prototype.workerReady = function() {
  if( !--this.queuedWorkers ) {
    this.emit( 'ready', _.map( this.workers, this.getWorkerId) );
  }
};
...
Crawler.prototype.workerMessage = function( worker, msg ) {
  ...
  if( command==='ready' ) {
    ...
    // Emit the error
    this.emit( 'error', socialId, msg.error );
...
Crawler.prototype.splitData = function( data, socials, socialId ) {
  this.trace( 'Splitting source data across %s socials for "%s"', socials.length, socialId );
  var workerData = data;
...
*/
Crawler.prototype.run = function( type, options, status ) {
  options = options || {};
  status = status || {};
